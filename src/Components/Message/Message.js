import React from 'react'
import './Message.css'
function Message(props) {
    return (
        <div>
            <div className={props.isRight?"message-container-right":"message-container-left"}>
                <div className="message-profile-photo">
                    <img width={50} height={50} style={{ borderRadius: '50%' }} src="data:image/png;base64," alt="" />
                </div>
                <div className="name-chat-container">
                    <div className="name">
                        <p>Ajmal</p>
                    </div>
                    <div className="chat">
                        <p>how are you guys.dvnjfjvnjenvrbvuiernvierbu.</p>
                    </div>
                </div>
            </div>
        </div>
    )
}

export default Message
